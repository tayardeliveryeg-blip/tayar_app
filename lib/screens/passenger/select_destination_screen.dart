...
class _SelectDestinationScreenState extends State<SelectDestinationScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          // ====== خانة بحث موحّدة: زرار البحث + خانة الكتابة + زرار اختار
          // من الخريطة، التلاتة جوه إطار/خلفية واحدة متلاصقة (زي جوجل
          // مابس)، بدل ما كانوا تلات عناصر منفصلة بينهم مسافة ======
          Padding(
  ...
  Widget _buildResultsArea() {
    ...
    if (_error != null) {
      return EmptyState(
        icon: _isSearchFailure ? Icons.wifi_off_rounded : Icons.search_off_rounded,
        title: _error!,
      );
    }
